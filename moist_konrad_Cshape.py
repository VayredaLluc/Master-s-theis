...
def moist_adiabat(T_s,T_rad,atmosphere):
    ...
    #If the radiation is worming the lower atmosphere just keep the radiative profile
    if T_moist_ad[0]<T_rad[0]:
        T_con = T_rad

    #Else, when convection warms the atmosphere 
    else:
...
def coldpoint(T,p):
    #iminT = np.where(np.diff(T)>=0)[0][0]
    iminT = np.where(T == np.min(T))[0][0]
    iminp = np.where(p <= 5e4)[0][0]
    imaxp = np.where(p <= 10e2)[0][0]
    ihpa = np.where(p <= 125e2)[0][0]
...
def manabe_rh(rhs, p, itop):
    p_uth = p[itop]
    rh_uth = 0.9*rhs*np.exp(-np.pi*np.log(p/p_uth)**2)
    #rh_mb = rhs*((p/p[0])-0.02)/(1-0.02)
    rh_mb = rhs*p/p[0]
    rh = np.maximum(rh_uth,rh_mb)
    #rh = np.maximum(rh,0.0001)
    return rh
...
def opt_column_water_to_rh(M_water,T_atm,p,ph,itop):
    ...
    rh_opt = manabe_rh(rhs_opt, p, itop)
    return rh_opt
...
def RCPE_step_DSE(timestep,
    ...
    sw_d_sfc = radiation['sw_flxd'][0,0]
    lw_d_sfc = radiation['lw_flxd'][0,0]
    ...
    if constrain_RH == True:
        if RH[0] > 0.99:
            RH = manabe_rh(0.99, atmosphere['plev'], cold_point)
            
        if RH[0] < 0.3:
            RH = manabe_rh(0.3, atmosphere['plev'], cold_point)
        
    atmosphere['H2O'][0] = rh_to_vmr(RH,atmosphere['T'][0],atmosphere['plev'],cold_point)
    RH = konrad.physics.vmr2relative_humidity(atmosphere['H2O'][0],atmosphere['plev'],atmosphere['T'][0])
    
    return atmosphere,surface,radiation,net_rad_surface,atm_rad,sw_d_sfc,lw_d_sfc,T_atm_low,E_imbalance,prec_mass/timestep,prec_heating,RH,cold_point,prec_eff,M_w